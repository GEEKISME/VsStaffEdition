...
public class M1Key {
    ...
    public static  Map<String, byte[]> CalculateKey(byte[] id){
        String hexCardSnr = Utils.bytesToHexString(id).toUpperCase();
...
//        Log.i(TAG, "CalculateKey: hexCardSnr = " + hexCardSnr);
        ...
        for (int i = 0;i < 2;i ++){
            ...
            if (!cardtype) {
                ...
                map.put(Constants.KEYA, KeySysByte);
            } else {
                map.put(Constants.KEYB, KeySysByte);
            }
        }
